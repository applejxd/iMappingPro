...
- 🎯 **リアルタイム AR スキャン**: トラッキング初期化ガイドとカウントダウンを経て記録を開始。計測中は LiDAR メッシュのプレビューと、録画開始地点への座標軸を表示 (表示専用で保存データには含まれません)
...
| 2 | スキャン開始 | 「開始」でトラッキング初期化ガイド (標準の coaching overlay) → 3-2-1 カウントダウンの後に記録が始まる。記録中は LiDAR メッシュ (ワイヤーフレーム) がオーバーレイされる |
| 2-a | 原点確認 | 録画開始地点に座標軸 (X: 赤 / Y: 緑 / Z: 青) が表示される。計測中 (スキャン中・一時停止中) のみ表示され、待機中・保存中は消える。メッシュ・点群には保存されない |
