...
ワールド原点のリセットとみなしてフレームを破棄する。判定には採用済みキーフレームではなく
直前に受信したフレームを用いる。
このため先頭数フレームを後処理で除外する必要はない。

スキャン中にセッションが中断された場合（アプリのバックグラウンド遷移など）、
中断明けのワールド座標系が中断前と一致する保証がないため、キャプチャを自動停止してユーザに通知する。
1 枚も採用していない状態での中断は、単に開始条件の待ち直しとなる。
