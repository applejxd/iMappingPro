...
- **記録開始フローの刷新**（初回スキャンの「姿勢の不連続」対策）
  - トラッキング初期化ガイドとして標準の `ARCoachingOverlayView` を表示
  - 開始前に 3-2-1 カウントダウンを挟み、トラッキングが `normal` でない間は
    カウントを進めない（`ScanState.preparing` を追加、カウント中は「中止」可能）
  - 記録開始直後（先頭 30 キーフレーム以内）の不連続は、エラーにせず原点を
    取り直して計測をやり直す（最大 3 回・取得済みフレームは破棄して通知）
...
- LiDAR メッシュのプレビューを計測中（スキャン中・一時停止中）のみ表示するよう変更。
  初期化中にシーン理解の可視化を有効にすると GPU 負荷でトラッキングの収束が遅れるため
- **スキャン中のカクつきを改善**
